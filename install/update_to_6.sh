...
select_node(){
    ...
    if [ ! -f $public_file ];then
        download_file $public_file https://raw.githubusercontent.com/zeronxdev/aapanel/main/install/public.sh
    fi
...
install_python(){
	...
	if [ -f $pyenv_path/pyenv/bin/python ];then
		...
		if [ "$is_package" = "" ];then
			wget -T 5 -O $pyenv_path/pyenv/pip.txt https://raw.githubusercontent.com/zeronxdev/aapanel/main/install/pip.txt
	...
	if [ "${os_version}" != "" ];then
		...
		#wget -O $pyenv_file $download_Url/install/pyenv/pyenv-${os_type}${os_version}-x${is64bit}.tar.gz -T 10
  		wget -O $pyenv_file https://github.com/zeronxdev/aapanel/releases/download/pyenv/pyenv-${os_type}${os_version}-x${is64bit}.tar.gz -T 10
	...
	#wget -O $python_src $download_Url/src/Python-${py_version}.tar.xz -T 5
 	wget -O $python_src https://github.com/zeronxdev/aapanel/releases/download/python/Python-${py_version}.tar.xz -T 5
	...
	cd ~
	rm -rf $python_src_path
	wget -T 5 -O $pyenv_path/pyenv/bin/activate https://raw.githubusercontent.com/zeronxdev/aapanel/main/install/activate.panel
	wget -T 5 -O $pyenv_path/pyenv/pip.txt https://raw.githubusercontent.com/zeronxdev/aapanel/main/install/pip.txt
...
update_panel(){
    wget -T 5 -O /tmp/panel.zip https://raw.githubusercontent.com/zeronxdev/aapanel/main/install/panel6_en.zip
    ...
    if [ "${check_bt}" = "" ];then
        rm -f /etc/init.d/bt
        wget -T 20 -O /etc/init.d/bt https://raw.githubusercontent.com/zeronxdev/aapanel/main/install/bt6_en.init
        chmod +x /etc/init.d/bt
    fi
    rm -f /www/server/panel/*.pyc
    rm -f /www/server/panel/class/*.pyc
    if [ ! -f $setup_path/server/panel/config/config.json ];then
        wget -T 5 -O $setup_path/server/panel/config/config.json https://raw.githubusercontent.com/zeronxdev/aapanel/main/install/config.json
        wget -T 5 -O $setup_path/server/panel/config/dns_api.json https://raw.githubusercontent.com/zeronxdev/aapanel/main/install/dns_api.json
    fi
